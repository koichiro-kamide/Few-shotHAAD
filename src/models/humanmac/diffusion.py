...
class Diffusion:
    def __init__(self, noise_steps=1000,
        ...
        self.scheduler = 'Cosine'  # 'Cosine', 'Sqrt', 'Linear', 'Sigmoid'
    ...
    def inpaint_complete(self, step, x, prev_t, traj_dct, mode_dict):
        ...
        Args:
            ...
            traj_dct: DCT coefficient of the traj,
                    shape as [sample_num, n_pred, 3 * joints_num]
        ...
        x_prev_t_known = torch.matmul(self.idct[:, :self.n_pred],
                                      x_prev_t_known[:, :self.n_pred])  # return time domain for mask
        x_prev_t_unknown = torch.matmul(self.idct[:, :self.n_pred],
                                        x[:, :self.n_pred])
        x = torch.mul(mode_dict['mask'], x_prev_t_known) + torch.mul((1 - mode_dict['mask']), x_prev_t_unknown)  # mask

        # change n_pred
        x = torch.matmul(self.dct[:self.n_pred], x)
    ...
    def sample_ddim_progressive(self, model, traj_dct, traj_dct_mod, mode_dict, noise=None):
        ...
        Returns a generator contains x_{prev_t}, shape as [sample_num, n_pred, 3 * joints_num]
        """
        model.eval()
